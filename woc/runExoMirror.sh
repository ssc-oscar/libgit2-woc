...
# CLONE date per repo (repo -> objects-dir mtime, epoch): when these objects
# were last cloned/(re)synced into the kept mirror -- the as-of date of the data
# (NOT the grab run). Rsynced to da8 with the dumps.
while read -r r; do [ -d "$r/objects" ] && printf '%s;%s\n' "$r" "$(stat -c %Y "$r/objects" 2>/dev/null)"; done \
  < "$DST/repos.list" | pigz > "$DST/$base.p2cd.gz"
...
rsync -a "$DST"/$base.olist.*.gz "$DST"/$base.p2cd.gz "$DST"/$base.*.{blob,commit,tree,tag}.{bin,idx} "$RSYNC_DEST/$rsub/" \
