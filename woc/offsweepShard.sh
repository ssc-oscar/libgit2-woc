...
awk -F';' '{b[$5]+=$2; tot+=$2} END{for(r in b) print "B;"r";"b[r]; print "TOT;;"tot}' "$pre.blob.idx" > "/tmp/osw.bt.$m.$l"
...
awk -F';' -v M="$m.$l" '$1=="REVIEW"{print strftime("%F %T")" REVIEW "M" "$2" blob="$3"GB tree="$4"GB share="$5}' "/tmp/osw.dec.$m.$l" >> "$REVIEW" 2>/dev/null
